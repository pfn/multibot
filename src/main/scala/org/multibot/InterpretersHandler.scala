...
case class InterpretersHandler(
  ...
  def serve(implicit msg: Msg): Unit = msg.message match {
    ...
    case Cmd("*clj" :: Sanitize(m) :: Nil) => http.respond(sendLines).respondJSON(:/("www.tryclj.com") / "eval.json" <<? Map("expr" -> m)) {
    ...
    case Cmd("*hs" :: Sanitize(m) :: Nil) => http.respond(sendLines).respondJSON(:/("tryhaskell.org") / "eval" <<? Map("exp" -> m)) {
      case JObject(
      JField("success",
      JObject(
    ...
    case Cmd("*idris" :: Sanitize(m) :: Nil) => http.respond(sendLines).respondJSON(:/("tryidris.herokuapp.com") / "interpret" << compact(render("expression", m))) {
    ...
    case Cmd("*py" :: Sanitize(m) :: Nil) => http.respond(sendLines).respondJSON2(:/("try-python.appspot.com") / "json" << compact(render(("method", "exec") ~("params", List(pythonSession, m)) ~ ("id" -> "null"))),
    ...
    case Cmd("*groovy" :: Sanitize(m) :: Nil) => http.respond(sendLines).respondJSON(:/("groovyconsole.appspot.com") / "executor.groovy" <<? Map("script" -> m), true) {
